    ...
    def update_item_graph(self, i, kk):
        ...
        for j in range(num_users):
            ...
            if invertible(self.S[j]):
                ...
                if np.abs(np.dot(theta - theta1, self.items[kk, :])) < self.alpha * (
                        fracT(self.T[i]) + fracT(self.T[j])):
                    N0.append(j)
                for a in range(len(A)):
                    l = A[a]
                    if np.abs(np.dot(theta - theta1, self.items[l, :])) < self.alpha * (
                            fracT(self.T[i]) + fracT(self.T[j])):
                        N[a].append(j)
            else:
                N0.append(j)
                ...
                if invertible(self.S[i]) == True:
                    ...
                    for j in A:
                        if invertible(self.S[j]) == True:
                            ...
                            if np.abs(np.dot(theta1 - theta2, self.items[kk, :])) > self.alpha * (
                                    fracT(self.T[i]) + fracT(self.T[j])):
